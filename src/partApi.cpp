...
std::size_t PartApi::writeCallback(void* contents, size_t size, size_t nmemb, void* userdata) {
...
CURLcode PartApi::triggerRequest(CURL* curl) {
...
nlohmann::json PartApi::parseData(const std::string& response) {
    try {
...
std::string PartApi::formSearchPattern(const std::string& item) {
...
void PartApi::initGlobalCurl() {
    if (globalInit_) { return; }
    curl_global_init(CURL_GLOBAL_DEFAULT);
    globalInit_ = true;
}

void PartApi::cleanupGlobalCurl() {
    curl_global_cleanup();
    globalInit_ = false;
}

PartApi::PartApi(ThreadPool& cPool, Config& cConfig, Logger& cLogger) : pool_(cPool), config_(cConfig), logger_(cLogger) {
...
nlohmann::json PartApi::fetchDataPoint(std::string dataPoint, bool forceRefetch) {
    if (!forceRefetch) {
...
void PartApi::fetchExample(std::string dataPoint, UI::ApiPreviewState& state) {
