...
void Config::getAdditionalConfig(const nlohmann::json& j) {
    try {
...
std::string Config::readConfigFile(const std::filesystem::path& configPath) {
...
std::vector<AutoInv::SerializableMapping> Config::readSingleMappingFile(const std::filesystem::path& path) {
...
void Config::saveSingleMappingToFile(const std::vector<AutoInv::MappingNumber>& mappings, const std::filesystem::path& path) {
...
Config::Config(Logger& cLogger) : logger_(cLogger) {}

std::string Config::setConfigString(const std::filesystem::path& configPath) {
    if (configPath.empty()) {
...
AutoInv::LoadedMappings Config::readMappings() {
...
void Config::saveMappings(const std::vector<AutoInv::MappingNumber>& mappingsBom,
                          const std::vector<AutoInv::MappingNumber>& mappingsOrder) {
...
const std::string& Config::getQuantityColumn() const {
    return quantityColumn_;
}

const ApiConfig& Config::getApiConfig() const {
    return api_;
}

void Config::setApiArchiveBuffer(DB::ProtectedData<ApiResponseType>* responses) {
    api_.responses = responses;
}

nlohmann::json Config::getDummyJson() const {
    return api_.dummyJson;
}

std::string Config::getSearchPattern() const {
    return api_.searchPattern;
}

std::filesystem::path Config::getCsvPathOrder() const {
    return order_.defaultPath;
}
